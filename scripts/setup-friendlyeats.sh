...
Examples:
  $(basename "$0") -r https://github.com/DingJun1028/OmniJunAikey.git
  $(basename "$0") -d my-friendlyeats -r git@github.com:DingJun1028/OmniJunAikey.git
EOF
}
...
git add .
# Do not fail if there is nothing to commit (re-runs)
...
ensure_firebase_cli() {
  ...
  if (( need_install == 1 )); then
    ...
    if npm install -g firebase-tools@latest; then
      ...
      echo "Global install failed (permissions?). Fallback to npx is available."
      echo "You can either:"
