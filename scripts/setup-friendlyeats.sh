...
# Helpful on Windows Git Bash: ensure HOME exists
if [[ -z "${HOME:-}" ]]; then
  HOME="$(pwd)"
  export HOME
fi
...
# If branch errors occur, switch master->main manually.
...
git add .
# Avoid failure if nothing changed on re-run
...
ver_to_int() {
  # Convert semver x.y.z to integer xyyyzzz for comparison
...
ensure_firebase_cli() {
  ...
  if (( need_install == 1 )); then
    ...
    if npm install -g firebase-tools@latest; then
      ...
      echo "Global install failed (permissions?). Alternatives:"
      echo "  - Re-run with elevated perms (e.g. Windows PowerShell as Administrator)"
      echo "  - Use a Node version manager (nvm)"
      echo "  - Or run firebase via npx each time: npx firebase-tools@latest <command>"
      echo "Continuing with npx fallback for this session..."
...
# Standard interactive login (opens browser). For headless/SSH, use --no-localhost
